...
func TestAnalyzeQueryMicroBurstFlood(t *testing.T) {
	configLock.Lock()
	config.AbuseDetectionEnabled = true
	config.BlockedClients = []string{}
	config.BlockedClientsInfo = make(map[string]BlockedClientInfo)
	...
	// Send 99 fast queries (under 100/sec burst limit)
	for i := 0; i < 99; i++ {
		analyzeQuery(ip, domain, "NOERROR")
	}
	...
	if isBlocked {
		t.Fatal("Client should not be blocked at 99 queries")
	}

	// 100th query within the 1-second burst window should trigger auto-block
	analyzeQuery(ip, domain, "NOERROR")

	// Poll briefly for the background auto-block goroutine to finish
	blocked := false
	for i := 0; i < 25; i++ {
	...
	if !blocked {
		t.Fatal("Client was not blocked after 100 queries/sec micro-burst flood")
	}
}
