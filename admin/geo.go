...
var (
	geoCacheDir = filepath.Join(DataDir, "geo")
	geoMu       sync.Mutex
)

func initGeo() {
	if _, err := os.Stat(geoCacheDir); os.IsNotExist(err) {
...
func syncCountryIPs(countryCode string) error {
	...
	dest := filepath.Join(geoCacheDir, countryCode+".zone")
...
func GetCountryList() map[string]string {
	return map[string]string{
		...
		"CD": "Congo, Democratic Republic of", "CK": "Cook Islands", "CR": "Costa Rica", "CI": "Cote d'Ivoire", "HR": "Croatia",
		...
		"KP": "Korea, Democratic People's Rep of", "KR": "Korea, Republic of", "KW": "Kuwait", "KG": "Kyrgyzstan", "LA": "Lao People's Democratic Republic",
